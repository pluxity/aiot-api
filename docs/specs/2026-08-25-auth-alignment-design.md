...
| §6.3 수동 확인이 `/auth/refresh-token` 성공을 200 으로 적었으나 컨트롤러는 `noContent()` 즉 **204** 다 | **타당** — 이대로 검증하면 정상 동작을 실패로 판정한다 | §6.3 을 204 로 정정 |
...
| 리프레시 | `/auth/refresh-token` → **204** + 새 쿠키 (`ResponseEntity.noContent()`. 200 을 기대하면 정상 동작을 실패로 판정한다) |
