...
→ **기존 토큰의 서명은 그대로 검증된다.** 무효화는 §5 의 수명 변경 때문이지 라이브러리 교체 때문이 아니다.
...
### 5.1 기존 발급 토큰이 전부 무효화된다

서명 알고리즘과 secret 이 동일하므로 **라이브러리 교체만으로는 무효화되지 않는다.** 무효화되는 것은
**§2 의 수명 정정 때문**이다.
...
**즉 기존 토큰은 즉시 무효화되지 않는다.** 새로 발급되는 토큰만 10시간/10일이 된다.
이미 나가 있는 416일짜리 액세스 토큰은 그대로 살아 있다.

### 5.2 그래서 별도 조치가 필요하다

수명 정정만으로는 **이미 유출됐을 수 있는 416일짜리 토큰을 회수하지 못한다.**
완전히 끊으려면 **secret 을 교체해야 한다.**

| 선택지 | 효과 | 비용 |
|---|---|---|
| A. 수명만 정정 | 신규 토큰부터 10h. 기존 토큰은 최대 416일 잔존 | 없음 — 사용자 영향 0 |
| B. 수명 정정 + secret 교체 | **모든 기존 토큰 즉시 무효** | 전 사용자 재로그인 |

**권장: A 로 배포하고, B 는 별도 공지 후 점검 시간에 수행한다.**
`secret` 이 yml 평문에 있고 safers-api 와 **동일한 값**이라는 점(§9-2)까지 고려하면 B 는 언젠가 해야 한다.
다만 이번 리팩터링과 묶으면 회귀 원인이 섞인다.

### 5.3 Redis 잔여 데이터

기존 `refresh_token` 레코드의 TTL 이 27년으로 박혀 있다. `@TimeToLive` 는 저장 시점에 적용되므로
**기존 레코드는 저절로 줄어들지 않는다.** A 안으로 가더라도 아래는 수행한다.

```bash
redis-cli --scan --pattern 'refresh_token:*' | xargs -r redis-cli TTL   # 현황 확인
```

전량 삭제하면 **모든 사용자가 리프레시 불가**가 되어 액세스 토큰 만료 후 재로그인해야 한다.
A 안과 함께라면 액세스 토큰이 416일 살아 있으므로 체감 영향은 없으나, **B 안과 동시에 수행하는 것이 자연스럽다.**
...
- Redis TTL 잔여 데이터(§5.3)

## 7. 남는 한계 (인지 사항)

1. **액세스 토큰 무효화 수단이 여전히 없다.** 로그아웃해도 `exp` 까지 유효하다(§2.3).
   수명을 10시간으로 줄이면 노출 창이 416일 → 10시간으로 줄지만 0 은 아니다.
   해결하려면 denylist(Redis)나 짧은 액세스 토큰 + 잦은 갱신이 필요하다 — §9.
...
| # | 과제 | 조건 |
|---|---|---|
| 1 | JWT secret 을 yml 평문에서 환경변수/시크릿 매니저로 분리 | safers-api 와 동일 값이라는 점도 함께 해소. §5.2 B 안과 묶어서 |
