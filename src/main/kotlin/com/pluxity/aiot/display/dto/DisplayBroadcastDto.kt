...
@Schema(
    description =
        "전광판 송출 요청. presetId 와 message 중 정확히 하나, " +
            "displayIds 와 siteIds 중 하나 이상을 지정합니다",
)
data class DisplayBroadcastRequest(
    ...
    @field:Schema(description = "송출 대상 전광판 아이디 목록", example = "[1, 2]")
    val displayIds: List<Long>? = null,
    @field:Schema(description = "송출 대상 현장 아이디 목록. 해당 현장의 모든 전광판으로 송출합니다", example = "[1]")
    ...
    @JsonIgnore
    @Schema(hidden = true)
    @AssertTrue(message = "전광판 아이디 또는 현장 아이디 중 하나 이상은 필수 입니다.")
    fun isTargetValid(): Boolean = !displayIds.isNullOrEmpty() || !siteIds.isNullOrEmpty()
}

data class DisplayBroadcastSearchRequest(
    val page: Int = 1,
    val size: Int = 10,
...
@Schema(
    description =
        "전광판 송출 이력. 업체 API 가 장치 단건 처리이므로 이력도 장치 1건 단위로 남습니다. " +
            "한 번의 송출 요청이 전광판 N개를 대상으로 하면 이력 N건이 생성됩니다",
)
data class DisplayBroadcastResponse(
    ...
    @field:Schema(description = "송출 대상 전광판의 소속 현장 아이디", example = "1")
    val siteId: Long?,
