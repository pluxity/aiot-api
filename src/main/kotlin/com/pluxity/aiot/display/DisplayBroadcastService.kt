...
class DisplayBroadcastService {
    ...
    fun findById(broadcastId: Long): DisplayBroadcastResponse = DETAIL_SAMPLE.copy(id = broadcastId)

    companion object {
        private val RESULT_SAMPLES =
            DisplayService.SAMPLES.map { display ->
                val success = display.status == DeviceStatus.NORMAL
                BroadcastResult(
                    targetId = display.id,
                    targetName = display.name,
                    siteId = display.site?.id,
        ...
        private val SUMMARY_SAMPLES =
            listOf(
                DisplayBroadcastSummaryResponse(
                    id = 1L,
